...
import com.pocketstock.core.recommendations.card.dto.CardBenefitItem;
...
import com.pocketstock.core.recommendations.card.dto.CardRecommendationResponse;
import com.pocketstock.core.recommendations.card.dto.CardRow;
import com.pocketstock.core.recommendations.card.dto.TopCategory;
...
public class CardRecommendationService {
    ...
    private static final Map<String, List<String>> CATEGORY_MAP = Map.ofEntries(
        ...
        Map.entry("음식·숙박", List.of("음식점", "카페", "배달", "외식", "커피", "제과", "밀딜리버리", "배민", "주말 외식비", "One Pick 맛집")),
    ...
    public CardRecommendationResponse recommend(Long userId) {
        ...
        if (total.compareTo(BigDecimal.ZERO) == 0) {
            return new CardRecommendationResponse(List.of(), List.of());
        }
        ...
        // TOP 3 카테고리
        List<TopCategory> topCategories = spending.stream()
                .limit(3)
                .map(r -> new TopCategory(
                        r.getCategory(),
                        r.getAmount().multiply(BigDecimal.valueOf(100))
        ...
        List<CardRecommendationItem> recommendations = cards.stream().map(card -> {
            ...
            List<CardBenefitItem> matchedBenefits = benefits.stream()
                    .filter(b -> b.getBenefitCategory() != null && spendRatio.keySet().stream().anyMatch(cat ->
                            CATEGORY_MAP.getOrDefault(cat, List.of()).stream()
                                    .anyMatch(k -> b.getBenefitCategory().toLowerCase().contains(k.toLowerCase()))
                    ))
                    .map(b -> new CardBenefitItem(b.getBenefitCategory(), b.getBenefitDesc()))
                    .distinct()
                    .limit(3)
                    .toList();
        ...
        return new CardRecommendationResponse(topCategories, recommendations);
    }
