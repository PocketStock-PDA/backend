...
**Response Body** — `data` = `OrderHistoryResponse[]` 배열(페이지네이션 없음). `side`=BUY/SELL · `orderType`=LIMIT/MARKET(온주)·AMOUNT/QUANTITY/ALL(소수점) · `status`=FILLED/PENDING/QUEUED/REJECTED · `currency`=KRW/USD. **`quantity`는 금액(AMOUNT) 주문 시 null**, **`orderAmount`는 금액(AMOUNT) 주문의 주문 금액(그 외 null)**, **`price`는 주문가(즉시체결=체결가, 지정가 미체결=지정가)이며 체결가/지정가 미기록 시 null**. `stockName`은 없음(종목명은 holdings/종목상세로 매핑).
