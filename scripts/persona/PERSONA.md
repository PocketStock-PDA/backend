...
pocketStock 전 플로우(자산 연동 ~ 포트폴리오 ~ CMA)를 커버하는 팀 공용 테스트 계정.
...
|---|---|
| 이름 | 임혜진 |
| username | **test1234** (팀 공용 로그인) |
| 비밀번호 | **Qwer1234!** |
| 계좌 비밀번호(거래 인증) | **1234** (환전·매매 시 입력) |
...
자산 연동만 완료하고 pocketStock 서비스(가계부·CMA)는 아직 미이용 상태의 신규 유입 페르소나.
...
| 이름 | 이수현 |
| username | **suhyunlee** |
| 생년월일 | 1985-04-12 (만 41세) |
| 성별 | 여성 |
| 직업 | 주부 |
| 월 생활비 | 약 240만원 (통계청 200~300만원 구간) |
| 주 거래 | 신한은행 / 신한카드 |
| 가입일 | 2026-02-15 |

> 가계부 목표 미설정, CMA 미개설, 신한투자증권 계좌 없음. 첫 진입 시 초기 설정 플로우 검증용.
...
| account_id | 계좌 | 잔액 | 통화 | 상태 |
|---|---|---|---|---|
| 8 | 신한 입출금통장 | 1,280,000원 | KRW | 활성 |
| 9 | 신한 정기예금 | 5,000,000원 | KRW | 활성 (3.8%, ~2027-07) |
| 10 | 신한 적금 | 4,200,000원 | KRW | 활성 (4.2%, ~2026-12) |
| 11 | SOL트래블 외화예금 | USD 45.00 | USD | 활성 |
| 12 | 국민 입출금 | 18,500원 | KRW | 휴면 |
| 13 | 카카오뱅크 입출금 | 32,000원 | KRW | 휴면 |
| 14 | 우리 입출금 | 4,700원 | KRW | 휴면 |

---

### db-a-linked-cards.csv

| card_id | 카드명 | 종류 | 결제 계좌 |
|---|---|---|---|
| 4 | 처음체크 | CHECK | 신한 입출금(8) |
| 5 | 처음신용 | CREDIT | 신한 입출금(8) |
| 6 | SOL트래블체크 | CHECK | SOL트래블 외화(11) |
...
### db-a-card-transactions.csv (175건)
2026-03-01 ~ 2026-07-10. 통계청 200~300만원 구간 카테고리 비율 적용. is_roundup_collected=FALSE (CMA 미개설).
...
| 2026-05 | 약 236만원 |
| 2026-06 | 약 237만원 |
| 2026-07 (1~10일) | 약 69만원 |

---

## DB B — 해당 없음
CMA 미개설, 증권계좌 없음 → DB B 데이터 없음.

---

# 페르소나 3 — 박지영

자산 종류(은행·카드·포인트·타사 증권)를 폭넓게 연동한 30대 중반 주부. 신한 주거래 + 다수의 타사 기관·제휴 포인트로 자산 화면 다양성과 소비 분석을 두텁게 검증.
...
> 신한투자증권 미이용(증권계좌·CMA 없음). 가계부 이용 중이며 **6월은 예산 내로 절약한 달**(전 카테고리 예산 아래). 타사 자산(은행·카드·증권)과 제휴 포인트를 폭넓게 보유한 연동 다양성 페르소나.
...
### db-a-linked-institutions.csv (id 15~31)
...
### db-a-linked-bank-accounts.csv (id 15~25)

| account_id | 계좌 | 잔액 | 통화 | 상태 |
|---|---|---|---|---|
| 15 | 신한 입출금통장 | 9,400원 | KRW | 활성 (잔돈 데모 — 만원 미만) |
| 16 | 신한 정기예금 | 5,000,000원 | KRW | 활성 (3.6%, ~2027-09) |
| 17 | 신한 그린 적금 | 3,600,000원 | KRW | 활성 (4.0%, **~2026-07-20 만기 30일 미만**) |
| 18 | 신한 쏠편한 적금 | 1,800,000원 | KRW | 활성 (3.7%, ~2027-12) |
| 19 | SOL트래블 외화예금 | USD 38.50 | USD | 활성 |
| 20 | 국민 일반 입출금 | 23,400원 | KRW | 휴면 |
| 21 | KB 정기예금 | 2,000,000원 | KRW | 활성 (만기 2026-03 지남) |
| 22 | 카카오뱅크 입출금 | 4,800원 | KRW | 활성 (잔돈 데모 — 오천원 미만) |
| 23 | 우리 입출금 | 8,200원 | KRW | 휴면 |
| 24 | 토스뱅크 입출금 | 900원 | KRW | 활성 (잔돈 데모 — 천원 미만) |
| 25 | 토스뱅크 키워봐요 적금 | 1,500,000원 | KRW | 활성 (3.9%, ~2027-10) |

> 타사 입출금 4개 중 2개(국민·우리) 휴면. 신한 적금 2개 중 1개(그린 적금)는 만기 임박.

---

### db-a-linked-cards.csv (id 7~11)

| card_id | 카드명 | 종류 | 결제 계좌 | 사용 |
|---|---|---|---|---|
| 7 | 신한카드 처음 체크 | CHECK | 신한 입출금(15) | 활성 (소액 다건) |
| 8 | 신한카드 Mr.Life | CREDIT | 신한 입출금(15) | 활성 (고정·중액) |
| 9 | KB국민 탄탄대로 체크카드 | CHECK | 국민 입출금(20) | 보유만 (거래 0건) |
| 10 | 현대카드 ZERO Edition3 | CREDIT | 신한 입출금(15) | 보유만 (거래 0건) |
| 11 | 롯데카드 LOCA 365 | CREDIT | 카카오뱅크(22) | 보유만 (거래 0건) |

> 카드 사용 내역은 신한 체크/신용 2장에만 기록. 타사 카드 3장은 연동만 된 보유 상태.

---

### db-a-linked-points.csv (id 3~9)
...
### db-a-external-holdings.csv (id 6~9)
토스증권(inst 21) 보유. 소수점 2종 + 온주 2종, 평가액 합 ≈ 39만원(큰 금액 아님).

| 종목 | 수량 | 평가액 |
|---|---|---|
| 삼성전자 (005930) | 0.7주 | 51,800원 |
| NAVER (035420) | 0.3주 | 58,500원 |
| LG전자 (066570) | 1주 (온주) | 95,000원 |
| SK하이닉스 (000660) | 1주 (온주) | 186,000원 |

---

### db-a-card-transactions.csv (274건, id 229~502)
2026-03-01 ~ 2026-07-07. 통계청 200~300만원 구간 12개 카테고리 비율 적용. is_roundup_collected=FALSE (CMA 미개설).
...
| 2026-04 | 약 245만원 |
| 2026-05 | 약 236만원 |
| 2026-06 | 약 230만원 (예산 2,420,000 대비 95% — 절약 달) |
| 2026-07 (1~7일) | 약 52만원 |

> 카드 배분: 신한 체크 / 신한 신용. **주류·담배 카테고리는 맥주·와인 등 가벼운 주류만, 담배 가맹점 제외.**
> 6월은 모든 카테고리가 예산(아래 budget_goals) 95% 수준 → "초과" 배지 없이 절약금 발생.

---

### db-a-budget-goals.csv (id 12~23)
6월(2026-06) 카테고리별 목표. 합계 2,420,000원 (5월 지출 기준 만원 올림 = 자동설정값과 동일). 6월 실제 지출이 모든 카테고리에서 이 목표 아래라 초과 없음.

| 카테고리 | 목표 | 6월 지출 |
|---|---|---|
| 식료품·비주류음료 | 420,000 | 399,000 |
| 음식·숙박 | 420,000 | 399,000 |
| 주거·수도·광열 | 400,000 | 380,000 |
| 기타상품·서비스 | 220,000 | 209,000 |
| 교통 | 210,000 | 199,500 |
| 보건 | 180,000 | 171,000 |
| 정보통신 | 140,000 | 133,000 |
| 오락·문화 | 120,000 | 114,000 |
| 의류·신발 | 110,000 | 104,500 |
| 가정용품·가사서비스 | 90,000 | 85,500 |
| 교육 | 70,000 | 66,500 |
| 주류·담배 | 40,000 | 38,000 |
| **합계** | **2,420,000** | **2,299,000 (절약금 121,000)** |

> `getGoals`는 `category IS NOT NULL` 행만 합산 → 전체 합계 행 불필요.

---

### db-a-budget-savings.csv (id 2)
절약 목표 100,000원 / is_collect_agreed=FALSE (절약금 모으기 미동의 → 온보딩 CTA 노출). 절약금(=예산-지출 121,000원)은 월말 CMA 이체 안내로 표시.
...
## SQL 시드
`backend/scripts/mysql-a-persona-seed-jiyoung.sql` (DB A 단일). CSV에 없는 **account_passwords**(거래 비번 1234) 포함. docker-compose `initdb.d`에 `06-persona-jiyoung.sql`로 마운트됨.
